...
class InMemSockets():
    ...
    def drop_user(self,user_id,idx):
        self.items = [_ for _ in self.items if not ((_[0] == user_id) and (_[2] == idx))]
...
def decode_token(token,session)->UserInfo:
    decoded = jwt.decode(token, key=MASTER_SECRET_KEY, algorithms=['HS256', ])
...
async def login(form_data: Annotated[OAuth2PasswordRequestForm, Depends()],
    ...
    user_info = session.query(UserInfo).filter(UserInfo.name == form_data.username).first()
    if not user_info:  
...
@app.get("/conv")
async def list_conv(current_user: Annotated[UserInfo, Depends(get_current_user)],
              session: Session = Depends(get_db)):
    """list the conversations"""

    q = session.query(ConvPrivacyMembers).filter(ConvPrivacyMembers.user_id==current_user.id)
...
async def websocket_endpoint(websocket: fastapi.WebSocket):
    ...
    try:
        ...
        logger.info("got data , %s",len(data))  # not logging data, because it contains the secure token of the user
...
async def broadcast_message_to_users(from_user:UserInfo,convo_id,user_ids:List[int], message:str):
    coros = []
...
async def new_msg(msg:MsgInputSchema,
    ...
    privacycheck = session.query(ConvPrivacyMembers).filter((
        ConvPrivacyMembers.user_id==user.id) & (
        ConvPrivacyMembers.conv_id==msg.conversation_id)).count()
    ...
    if privacycheck:
        ...
        session.add(cm)
        session.commit()
        cc = Convchanges(ts=time.time(),conv_id=convo.id,change_type="message",change_id=cm.id)

        session.add(cc)
        session.commit()

        members = session.query(ConvPrivacyMembers).filter((
            ConvPrivacyMembers.conv_id==msg.conversation_id)).all()
