...
class Convo(ScrollableContainer):
    ...
    async def append_other_msg(
        self, msg_id, content, sendernick, timestr, auto_scroll=True
    ):
        other_msg = OtherMsg("")
...
class DiscussionsWidgets(Static):
    ...
    def compose(self):
        ...
        with ContentSwitcher(
            initial=self.initial_discussion_id, classes="discussion_switcher"
        ):
...
class DiscussionWidget(Static):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.task2: asyncio.Task = asyncio.create_task(
            self.update_response_loop(self.aqueueoutput)
        )
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical():
            with ContentSwitcher(
                initial=self.switcher_ids[0], classes="convo_switcher"
            ):
    ...
    async def update_response_loop(self, aqueue: asyncio.Queue):
        ...
        while True:
            ...
            if len(msg_info) == 1:
                ...
                log_path = log_msg[
                    "path"
                ]  # / separated path ; can be empty string, it means the root.
                log_value = log_msg[
                    "value"
                ]  # the value of the log, its a dict with key values.
...
class ContactPanel(Static):
    ...
    def compose(self) -> ComposeResult:
        yield ListView(*[], name="contact_list")
    ...
    def on_list_view_selected(self, selected_msg: ListView.Selected):
        ...
        if convo_id in self.convo_id_to_widget_id:
            ...
            self.post_message(
                self.Selected(convo_id, self.convo_id_to_widget_id[convo_id])
            )
        selected_msg.stop()
...
class PP(App):
    ...
    async def on_login_success(self, usernameused):
        ...
        for convo in convodata:
            ...
            await cp.insert_in_contact_list(
                f"@ {convo['label']}", convo["id"], discussion_widget_id
            )
        ...
        for child in cs.children:
            child.display = (
                bool(dws.initial_discussion_id)
                and child.id == dws.initial_discussion_id
            )
        ...
        for convo in convodata:
            ...
            for c in sorted(convo_content, key=lambda x: x["ts"]):
                ...
                if self.myuser_id == c["sender"]:
                    ...
                    await convowidget.append_other_msg(
                        c["id"],
                        c["content"],
                        self.get_nick_name(c["sender"]),
                        strts,
...
def main() -> None:
    ...
    host = os.getenv("PPN_HOST", "http://127.0.0.1:8000/")
    ws_host = os.getenv("PPN_WSHOST", "ws://127.0.0.1:8000/")
